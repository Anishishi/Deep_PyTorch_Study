...
class Anno_xml2list(object):
  ...
  def __call__(self, xml_path, width, height):
    ...
    ret : [[xmin, ymin, xmax, ymax, label_ind], ...]
      物体のアノテーションデータを格納したリスト。画像内に存在する物体数分のだけの要素を持つ
    """
    ...
    for obj in xml.iter('object'):
      ...
      #resに[xmin, ymin, xmax, ymax, label_ind]をたす
      ret += [bndbox]
...
#入力画像の前処理をするクラス
from utils.data_augumentation import Compose, ConvertFromInts, \
...
class DataTransform():
  ...
  def __init__(self, input_size, color_mean):
    self.data_transform = {
        'train':Compose([
            ...
            ToPercentCoords(), #アノテーションデータを0~1に規格化する
  ...
  def __call__(self, img, phase, boxes, labels):
    """
    Parameters
    Phase : 'train' or 'val' 前処理モードの指定。dataaugumentationの有無が変わる
    """
...
class VOCDataset(data.Dataset):
  ...
  Attributes
  img_list : リスト 画像のパスを格納したリスト
  anno_list : リスト アノテーションへのパスを格納したリスト
  phase : 'train' or 'val'
  ...
  def pull_item(self, index):
    ...
    #色チャネルの順番がBGRになっているのでRGBに。さらに(高さ、幅、色チャネル)を(色チャネル、高さ、幅)に
